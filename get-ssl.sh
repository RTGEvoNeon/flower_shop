...
# Скрипт для получения SSL сертификата для эдемский-сад.рф

echo "🚀 Настройка SSL для эдемский-сад.рф"
...
if [ $? -eq 0 ]; then
    ...
    echo "🌐 Ваш сайт доступен по адресу: https://эдемский-сад.рф"

else
    echo "❌ Ошибка получения SSL сертификата"
    echo "Убедитесь что:"
    echo "- Домен эдемский-сад.рф направлен на ваш сервер"
