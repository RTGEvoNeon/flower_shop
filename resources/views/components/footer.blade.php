<!-- Footer -->
<footer class="bg-gray-900 text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            
            <!-- Контакты -->
            <div>
                <h3 class="text-lg font-semibold mb-6">Контакты</h3>
                <div class="space-y-4">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                        </svg>
                        <span>+7 (953) 292-92-46</span>
                    </div>
                    
                    <!-- <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        <span>info@itulip.ru</span>
                    </div> -->
                    
                    <div class="flex items-start">
                        <svg class="w-5 h-5 mr-3 mt-1 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        </svg>
                        <span>г. Брянск<br>доставка по всему городу</span>
                    </div>
                    
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span>Выдача букетов: Пн-Сб 8:00 - 22:00</span>
                    </div>
                </div>
            </div>
            
            <!-- Каталог -->
            <div>
                <h3 class="text-lg font-semibold mb-6">Каталог</h3>
                <ul class="space-y-3">
                    <li><a href="/products" class="text-gray-300 hover:text-white transition-colors">Все букеты</a></li>
                    <li><a href="/products?category=bouquets" class="text-gray-300 hover:text-white transition-colors">Букеты</a></li>
                    <li><a href="/products?category=wedding" class="text-gray-300 hover:text-white transition-colors">Свадебные</a></li>
                    <li><a href="/products?category=seasonal" class="text-gray-300 hover:text-white transition-colors">Сезонные</a></li>
                    <li><a href="/products?category=luxury" class="text-gray-300 hover:text-white transition-colors">Премиум</a></li>
                    <li><a href="/custom-bouquet" class="text-gray-300 hover:text-white transition-colors">Собери свой букет</a></li>
                </ul>
            </div>
            
            <!-- Информация -->
            <div>
                <h3 class="text-lg font-semibold mb-6">Информация</h3>
                <ul class="space-y-3">
                    <li><a href="/about" class="text-gray-300 hover:text-white transition-colors">О нас</a></li>
                    <li><a href="/delivery" class="text-gray-300 hover:text-white transition-colors">Доставка и оплата</a></li>
                    <li><a href="/care" class="text-gray-300 hover:text-white transition-colors">Уход за цветами</a></li>
                    <li><a href="/guarantee" class="text-gray-300 hover:text-white transition-colors">Гарантии</a></li>
                    <li><a href="/contacts" class="text-gray-300 hover:text-white transition-colors">Контакты</a></li>
                </ul>
            </div>
            
            <!-- Социальные сети -->
            <div>
                <h3 class="text-lg font-semibold mb-6">Мы в соцсетях</h3>
                <div class="flex space-x-4 mb-6">
                    <a href="https://t.me/FlowersMindale" class="w-10 h-10 bg-pink-600 rounded-full flex items-center justify-center hover:bg-pink-700 transition-colors">
                        <span class="text-sm font-bold">Telegram</span>
                    </a>
                    <!-- <a href="#" class="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center hover:bg-green-700 transition-colors">
                        <span class="text-sm font-bold">WA</span>
                    </a>
                    <a href="#" class="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center hover:bg-purple-700 transition-colors">
                        <span class="text-sm font-bold">IG</span> -->
                    </a>
                </div>
                
                <div class="text-sm text-gray-400">
                    <p class="mb-2">Подпишитесь на нас!</p>
                    <p>Новости, акции и красивые букеты в вашей ленте</p>
                </div>
            </div>
        </div>
        
        <!-- Нижняя часть -->
        <div class="border-t border-gray-800 mt-12 pt-8">
            <div class="flex flex-col md:flex-row justify-between items-center">
                <div class="mb-4 md:mb-0">
                    <p class="text-sm text-gray-400">
                        © 2025 Mindale. Все права защищены.
                    </p>
                </div>
                
                <div class="flex space-x-6">
                    <a href="/privacy" class="text-sm text-gray-400 hover:text-white transition-colors">
                        Политика конфиденциальности
                    </a>
                    <a href="/terms" class="text-sm text-gray-400 hover:text-white transition-colors">
                        Публичная оферта
                    </a>
                </div>
            </div>
            
            <div class="mt-6 text-center">
                <p class="text-xs text-gray-500">
                    ИП Редин Дмитрий Витальеивч<br>
                    <!-- ИНН: 123456789012 ОГРНИП: 123456789012345<br>
                    Адрес: г. Москва, ул. Цветочная, д. 1 -->
                </p>
            </div>
        </div>
    </div>
</footer>
